...
async function fetchPage(
  url,
  { timeoutMs = 8000, maxChars = 200_000, signal } = {}
) {
    ...
    const kick = () => {
      ...
      while (active < limit && i < items.length) {
        ...
        Promise.resolve()
          ...
          .catch(
            (e) => (results[idx] = { error: String((e && e.message) || e) })
          )
          .finally(() => {
            active--;
            kick();
...
async function researchWeb(
  ...
  const skipRe =
    /(youtube\.com|x\.com|twitter\.com|facebook\.com|reddit\.com)/i;
  const crawl = [];
  ...
  return {
    ...
    query,
    results: enriched,
    fetched: enriched.filter((x) => x.page && x.page.ok).length,
  };
}
