...
function pickParagraphs(html, query, maxChars = 20000) {
  ...
  const rawParas = body.split(/<\/(?:p|li|div)>/i).map(stripTags).map(s => s.trim()).filter(s => s.length > 60);
  const q = tokenize(query);
  const scored = rawParas.map(p => ({ p, s: scoreText(p, q) }))
    .filter(x => x.s > 0)
    .sort((a,b) => b.s - a.s)
    .slice(0, 30);
  const picked = [];
...
export async function answerWeb(
  ...
  for (const { r, html } of pages) {
    const content = pickParagraphs(html, query, Math.max(2000, Math.floor(maxContextChars / pages.length)));
    if (!content) continue;
  ...
  try {
    const r = await llmChat({ messages, maxTokens: maxAnswerTokens, temperature: 0.1, timeoutMs: 60000 });
