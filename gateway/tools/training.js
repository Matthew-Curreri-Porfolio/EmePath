...
function buildQuestionSynthPrompt(
  topic,
  n = 3,
  difficulty = 'hard',
  evidence = ''
...
async function synthQuestions(
  ...
  try {
    if (mode !== 'local') {
      const rr = await researchWeb(topic, {
        base,
        num: Math.min(5, num || 5),
        fetchNum: Math.min(3, fetchNum || 3),
        concurrency: Math.min(2, concurrency || 2),
        site,
        lang,
        safe,
        fresh,
  ...
  try {
    ...
    const arr = Array.isArray(j.questions)
      ? j.questions.filter((x) => typeof x === 'string' && x.trim()).slice(0, 3)
      : [];
    return arr.length
      ? arr
      : [
          `What are the core trade-offs in ${topic}, and how to verify a chosen approach in practice?`,
        ];
  } catch {
    return [
      `What are the core trade-offs in ${topic}, and how to verify a chosen approach in practice?`,
...
export async function trainLoop(
  ...
  for (let it = 0; it < clamp(iterations, 1, 10); it++) {
    const qs = await synthQuestions(topic, {
      mode,
      base,
      num,
      fetchNum,
    ...
    for (const q of chosen) {
      const insights = await insightsEngine(q, {
        mode,
        base,
        num,
        fetchNum,
      ...
      const debate = await debateEngine(q, {
        mode,
        useInsights: true,
        rounds: 2,
        base,
      ...
      const plan = await planEngine(
        { query: q, target: 'general', envOs: 'linux', risk: 'medium' },
        {
          mode,
          base,
          num,
      ...
      let solution;
      try {
        solution = JSON.parse(sol?.content || '{}');
      } catch {
        solution = { solution: String(sol?.content || '').trim() };
      }

      const gradeMsgs = buildGraderPrompt(
        q,
        solution,
        debate,
        plan,
      ...
      let grade;
      try {
        grade = JSON.parse(grd?.content || '{}');
      } catch {
        grade = {
          score: 0.5,
          methodology: {
            improvements: [String(grd?.content || '').slice(0, 400)],
          },
        };
      }
      ...
      // Update methodology
      const methMsgs = buildMethodologySynthPrompt(
        topic,
        methodology,
        grade?.methodology?.improvements || []
      );
      const meth = await llmChat({
        messages: methMsgs,
        temperature: 0.2,
        maxTokens: 800,
        timeoutMs: 45000,
      ...
      try {
        methodology = JSON.parse(meth?.content || '{}');
      } catch {}
    }
  }
  ...
  if (persist && typeof setLongTerm === 'function' && userId && workspaceId) {
    const payload = {
      ...
      methodology,
      timestamp: Date.now(),
      exemplars: runs.map((r) => ({
        question: r.question,
        solution: r.solution,
        grade: r.grade?.score ?? null,
      })),
    };
    try {
      setLongTerm(userId, workspaceId, JSON.stringify(payload, null, 2));
    } catch {}
  }
    ...
    try {
      ...
      for (const r of runs) {
        const example = {
          ...
          context: {
            sources: r.insights?.sources || [],
            plan: r.plan?.plan || null,
            verdict: r.debate?.debate?.verdict || null,
          },
