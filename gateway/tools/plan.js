...
function pickLocalHits(index, query, k = 6) {
  ...
  return scored.slice(0, Math.max(1, k)).map(({f,score}) => ({ path:f.path, score, snippets: makeSnippets(f.text, terms) }));
}

function buildEvidence({ mode, insights, localIndex, query, localK=6, maxContextChars=20000 }) {
  const blocks = [];
  const sources = [];
  ...
  if ((mode === 'local' || mode === 'hybrid') && (!insights || !insights.sources?.length)) {
    ...
    for (const h of hits) {
      ...
      sources.push({ id, title:h.path, path:h.path, snippet: snip.slice(0,240) });
    }
  }
...
export async function planEngine(
  ...
  let insights;
  try {
    insights = await insightsEngine(query, { mode, base, num, fetchNum, concurrency, site, lang, safe, fresh, localIndex, localK, maxContextChars, maxAnswerTokens: 500 });
  } catch {}
  const { evidenceText, sources } = buildEvidence({ mode, insights: (insights&&insights.ok) ? insights : null, localIndex, query, localK, maxContextChars });
