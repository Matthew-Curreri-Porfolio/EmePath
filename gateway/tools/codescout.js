...
function fetchTextFromHtml(html) {
  ...
  // <pre><code>...</code></pre>
  const preCodeRe = /<pre[^>]*>\s*(?:<code[^>]*>)?([\s\S]*?)(?:<\/code>)?\s*<\/pre>/gi;
  let m;
  ...
  while ((m = codeRe.exec(cleaned))) {
    ...
    if (body.split("\n").length >= 3 && body.trim().length >= 16) preBlocks.push(body.trim());
  }
...
function guessLang(code, hint = "") {
  ...
  if (/\bclass\s+\w+\s*\{/i.test(code) && /public\s+static\s+void\s+main/.test(code)) return "java";
  if (/^\s*#include\b|\bint\s+main\s*\(/.test(code)) return "c";
  if (/^\s*fn\s+main\s*\(\)/.test(code) || /\bextern\s+crate\b/.test(head)) return "rust";
  if (/package\s+main|func\s+main\(\)/.test(head)) return "go";
  if (/using\s+System;|Console\.WriteLine/.test(head)) return "csharp";
  if (/def\s+\w+\s*\(|import\s+\w+|print\s*\(/.test(head) && !/console\.log/.test(head)) return "python";
  if (/(?:const|let|var)\s+\w+\s*=|function\s+\w+\s*\(|=>\s*\{/.test(head)) return "javascript";
  if (/console\.log\(|document\./.test(head)) return "javascript";
  if (/SELECT\s+.+\s+FROM\s+/i.test(code)) return "sql";
  if (/^\s*#!\/(bin\/bash|usr\/bin\/env\s+bash)/.test(code) || /\bcurl\s+https?:\/\//.test(head)) return "bash";
  return "";
}
...
function normalizeSnippet(s) {
  return s.trim().replace(/\s+$/gm, "").replace(/[ \t]+/g, " ");
}
...
async function codeScout(
  ...
  for (let i = 0; i < picked.length && snippets.length < maxSnippets; i++) {
    ...
    for (const block of codeBlocks) {
      ...
      seen.add(id);
      snippets.push({ id, lang: g || null, lines, url: r.url, title, snippet: norm });
