...
function parseArgs() {
  ...
  const out = { out: 'gateway/tools/curated/cache/docs.jsonl', limit: 20, pages: [], arxiv: null, pubmed: null, seeds: null };
  for (let i = 0; i < args.length; i++) {
    ...
    else if (a === '--seeds') out.seeds = args[++i] || 'gateway/tools/curated/seeds.json';
  }
  return out;
}
...
async function main() {
  ...
  if (opts.mdn) {
    try {
      ...
      for (const d of mdnDocs) { d.id = `mdn:${hashId(d.url)}`; add(d); }
  ...
  if (opts.arxiv) {
    try {
      ...
      for (const d of av) { d.id = `arxiv:${hashId(d.url)}`; add(d); }
  ...
  if (opts.pubmed) {
    try {
      ...
      for (const d of pv) { d.id = `pubmed:${hashId(d.url)}`; add(d); }
    ...
    try {
      ...
      if (seeds?.wikipedia?.pages) {
        for (const p of seeds.wikipedia.pages) {
          try {
            const d = await wikiFetchByTitle(p, { lang: seeds.wikipedia.lang || 'en' });
            d.id = `wikipedia:${hashId(d.url)}`; add(d);
          } catch (e) { console.error('seed wiki error', p, e?.message || e); }
        }
      ...
      if (seeds?.mdn?.limit) {
        ...
        } catch (e) { console.error('seed mdn error', e?.message || e); }
      }
      if (Array.isArray(seeds?.arxiv)) {
        for (const a of seeds.arxiv) {
          ...
          } catch (e) { console.error('seed arxiv error', a?.query, e?.message || e); }
        }
      }
      if (Array.isArray(seeds?.pubmed)) {
        for (const p of seeds.pubmed) {
          try {
            const pv = await fetchPubMed({ query: p.query, max: p.limit || 10 });
            for (const d of pv) { d.id = `pubmed:${hashId(d.url)}`; add(d); }
            console.log('pubmed seeds:', pv.length, p.query);
          } catch (e) { console.error('seed pubmed error', p?.query, e?.message || e); }
        }
      }
    } catch (e) {
