...
describe('Admin cache/log routes', () => {
  let app; let server; let agent; let token;
  ...
  beforeAll(async () => {
    ...
    let login = await agent.post('/auth/login').send({ username: 'admin', password: 'changethis', workspaceId: 'ws-admin' });
    if (login.status === 401) {
      const username = `admin_${Date.now()}`;
      const password = 'pw';
      db.createUser(username, password);
      login = await agent.post('/auth/login').send({ username, password, workspaceId: 'ws-admin' });
    }
  ...
  it('exposes cache stats and clear endpoints', async () => {
    const stats = await agent.get('/admin/cache/stats').set('Authorization', `Bearer ${token}`);
    expect(stats.status).toBe(200);
    expect(stats.body.ok).toBe(true);
    expect(stats.body.stats).toBeTruthy();

    const cleared = await agent.post('/admin/cache/clear').set('Authorization', `Bearer ${token}`).send({ expiredOnly: true });
  ...
  it('lists and fetches llm request logs', async () => {
    ...
    const c = await agent.post('/complete').send({ language: 'txt', prefix: 'ping', suffix: '' });
    expect([200, 502, 400]).toContain(c.status);
    const ch = await agent.post('/chat').send({ messages: [{ role: 'user', content: 'hello' }] });
    expect([200, 502]).toContain(ch.status);

    const logs = await agent.get('/admin/logs').set('Authorization', `Bearer ${token}`);
    ...
    if (logs.body.items.length > 0) {
      ...
      const detail = await agent.get(`/admin/logs/${id}`).set('Authorization', `Bearer ${token}`);
  ...
  it('filters logs by model/kind/date and supports paging/detail', async () => {
    ...
    // 2 completes for A
    await agent.post('/complete').send({ model: modelA, language: 'txt', prefix: 'alpha', suffix: '' });
    await agent.post('/complete').send({ model: modelA, language: 'txt', prefix: 'beta', suffix: '' });
    // 1 chat for B
    await agent.post('/chat').send({ model: modelB, messages: [{ role: 'user', content: 'hello' }] });

    // Model filter
    const byModel = await agent.get('/admin/logs').query({ model: modelA }).set('Authorization', `Bearer ${token}`);
    ...
    // Kind filter
    const byKind = await agent.get('/admin/logs').query({ kind: 'complete' }).set('Authorization', `Bearer ${token}`);
    ...
    const byDate = await agent.get('/admin/logs').query({ since, until }).set('Authorization', `Bearer ${token}`);
    expect(byDate.status).toBe(200);
    expect(byDate.body.ok).toBe(true);
    expect(Array.isArray(byDate.body.items)).toBe(true);

    // Paging with limit/offset
    const page1 = await agent.get('/admin/logs').query({ limit: 1, offset: 0 }).set('Authorization', `Bearer ${token}`);
    const page2 = await agent.get('/admin/logs').query({ limit: 1, offset: 1 }).set('Authorization', `Bearer ${token}`);
    ...
    const detail = await agent.get('/admin/logs').query({ detail: 1, limit: 1 }).set('Authorization', `Bearer ${token}`);
    expect(detail.status).toBe(200);
    expect(detail.body.ok).toBe(true);
    ...
    // Summary by model
    const sumModel = await agent.get('/admin/logs/summary').query({ group: 'model', since, until }).set('Authorization', `Bearer ${token}`);
    ...
    // Summary by date
    const sumDate = await agent.get('/admin/logs/summary').query({ group: 'date', since, until }).set('Authorization', `Bearer ${token}`);
    ...
    const sumModelDate = await agent.get('/admin/logs/summary').query({ group: 'model_date', since, until, limit: 5 }).set('Authorization', `Bearer ${token}`);
