...
export async function chatStreamUseCase(req, res, deps) {
  ...
  const BASE =
    (process.env.LLAMACPP_SERVER || '').replace(/\/$/, '') ||
    'http://127.0.0.1:8080';
  ...
  try {
    ...
    if (LOG_BODY)
      log({ id, event: 'upstream_body', type: 'chat_stream', payload });
    ...
    const status = (obj) => {
      try {
        res.write(
          `data: ${JSON.stringify({ id, event: 'status', ...obj })}\n\n`
        );
        res.flush?.();
      } catch {}
    };
    ...
    const hb = setInterval(() => {
      if (!sawData) status({ state: 'waiting' });
    }, 1000);
    const body = r.body;
    ...
    const latencyAll = Math.round(performance.now() - t0);
    const isAbort =
      e?.name === 'AbortError' || /timeout/i.test(String(e?.message || e));
