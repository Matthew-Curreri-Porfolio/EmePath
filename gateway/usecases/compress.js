...
function ensureAuth(req, res) {
  ...
  const workspaceId =
    req.session && req.session.workspaceId ? req.session.workspaceId : null;
...
function appendCorpus(userId, jsonlObjs) {
  ...
  const baseData =
    rec && rec.data && typeof rec.data === 'object' ? rec.data : {};
  ...
  if (keepTrainId) db.setTraining(userId, next, keepTrainId);
  else db.setTraining(userId, next);
...
async function compressShortToLongUseCase(req, res, deps) {
  ...
  let chatHistory =
    typeof body.chatHistory === 'string' && body.chatHistory.length > 0
      ? body.chatHistory
      : makeChatHistoryFromMemory(shortItems);
  ...
  const keepAlive =
    typeof body.keepAlive === 'string' ? body.keepAlive : undefined;
  ...
  try {
    ...
    res
      .status(502)
      .json({ ok: false, error: String(e.message ? e.message : e) });
    return;
  }
  ...
  const sumMsgs = [
    ...
    {
      role: 'user',
      content: shortItems.map((i) => '- ' + (i.content || '')).join('\n'),
    },
  ];
  let summary = '';
  try {
  ...
  if (log)
    log('compress.shortToLong', { userId: auth.userId, ws, appended, memid });
