...
export async function trainingPut(req, res) {
  ...
  const trainid = body.trainid && body.trainid.length > 0 ? body.trainid : undefined;
...
export async function trainingBuild(req, res) {
  const auth = ensureAuth(req, res); if (!auth) return;
  const includeShort = req.body && typeof req.body.includeShort === 'boolean' ? req.body.includeShort : true;
  const includeLong = req.body && typeof req.body.includeLong === 'boolean' ? req.body.includeLong : true;
  const ws = req.session && req.session.workspaceId ? req.session.workspaceId : 'default';
  const corpus = [];

  if (includeShort) {
    const items = db.listMemory(auth.userId, ws, 'short');
    for (const it of items) corpus.push({ id: it.memid, scope: 'short', text: it.content, createdAt: it.createdAt, updatedAt: it.updatedAt });
  }
  if (includeLong) {
    const items = db.listMemory(auth.userId, ws, 'long');
    for (const it of items) corpus.push({ id: it.memid, scope: 'long', text: it.content, createdAt: it.createdAt, updatedAt: it.updatedAt });
  }
  ...
  const rec = db.setTraining(auth.userId, next, existing && existing.trainid ? existing.trainid : undefined);
  return res.json(rec);
}
