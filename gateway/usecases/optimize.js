...
import { getMachineProfile, setMachineProfile, getUserProfile, setUserProfile } from '../db/hwStore.js';
...
export async function runHwOptimizeUseCase(req, res, deps){
  ...
  if (!modelArg) return res.status(400).json({ ok:false, error:'model is required (ollama id or /abs/path/model.gguf)' });

  const gguf = modelArg.startsWith('/') ? modelArg : (await resolveBaseGGUF(modelArg));
  if (!gguf) return res.status(404).json({ ok:false, error:'could not resolve model to a local .gguf path' });
