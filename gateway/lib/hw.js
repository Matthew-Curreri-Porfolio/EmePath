...
function cpuThreads(){ const n = os.cpus().length; return n >= 8 ? n - 1 : n; }
...
async function gpuInfo(){
  ...
  if (await cmdExists('nvidia-smi')){
    ...
    if (r.ok){
      ...
      const gpus = lines.map((ln,idx)=>{
        ...
        return { vendor:'nvidia', index:idx, name, memGiB: Number(memGiB.replace(/[^0-9.]/g,'')), compute: cc };
      });
  ...
  if (process.platform === 'darwin') {
    return { type:'apple', gpus:[{vendor:'apple', index:0, name:'Apple GPU', memGiB: 0}] };
  }
...
async function resolveFromToken(tok){
  ...
  for (const root of candidateRoots()) {
    for (const name of [tok, tok.startsWith('sha256-')? tok : `sha256-${tok}`]) {
...
export async function optimize({ model, deep=false, quick=false }){
  ...
  const batch = (gi.type==='nvidia' && memGiB>=24) ? 2048 : (gi.type==='nvidia' && memGiB>=12) ? 1536 : 1024;
  ...
  return {
    ...
    fileMB,
    layersGuess,
    threads, ctx, batch, ubatch, ngl,
    env,
    recommend: {
      server: {
        cmd: process.env.LLAMACPP_SERVER || './llama.cpp/build/bin/llama-server',
        args: ['-m', model, '--host','127.0.0.1','--port','8080','--ctx-size',String(ctx),'--batch-size',String(batch),'--ubatch-size',String(ubatch),'-ngl',String(ngl),'--threads',String(threads),'--flash-attn','auto'],
        env
      },
      cli: {
        cmd: process.env.LLAMACPP_CLI || './llama.cpp/build/bin/llama-cli',
        args: ['-m', model, '--n-predict','64','-ngl', String(ngl),'--threads', String(threads),'--flash-attn','auto'],
        env
      },
      finetune: {
        cmd: process.env.LLAMA_FINETUNE || './llama.cpp/build/bin/llama-finetune',
        args: ['-m', model,'--lora-out','/tmp/adapter.gguf','--lora-r','16','--lora-alpha','16','--epochs','1','--seq-len','2048','--lr','1e-4'],
        env
      },
      m2m: {
        serverHints: { temperature:0.2, top_p:0.8, repeat_penalty:1.05, ctx_size:ctx, batch, ubatch },
