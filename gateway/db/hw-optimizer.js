...
function argParse(){
  const a = process.argv.slice(2); const out = { mode:'server', quick:false, deep:false, host:'127.0.0.1', port:8080 };
  for (let i=0;i<a.length;i++){
    const k=a[i];
    if (k==='--model'){ out.model=a[++i]; }
    else if (k==='--mode'){ out.mode=a[++i]; }           // server|cli|finetune|m2m
    else if (k==='--quick'){ out.quick=true; }
    else if (k==='--deep'){ out.deep=true; }
    else if (k==='--host'){ out.host=a[++i]; }
    else if (k==='--port'){ out.port=toInt(a[++i],8080); }
    else if (k==='--epochs'){ out.epochs=toInt(a[++i],1); }
    else if (k==='--rank'){ out.rank=toInt(a[++i],16); }
    else if (k==='--alpha'){ out.alpha=toInt(a[++i],16); }
    else if (k==='--seq-len'){ out.seqLen=toInt(a[++i],2048); }
    else if (k==='--write'){ out.write=a[++i]; }
    else if (k==='--print-cmd'){ out.printCmd=true; }
  }
...
async function run(cmd, args=[], opts={}) {
  try {
    const { stdout, stderr } = await exf(cmd, args, { timeout: opts.timeoutMs || 90_000 });
    return { ok:true, stdout:String(stdout||''), stderr:String(stderr||''), code:0 };
  } catch (e) {
    return { ok:false, stdout:String(e.stdout||''), stderr:String(e.stderr||''), code:e.code ?? -1, err:e };
  }
}
...
function bytes(n){ return Math.max(0, Number(n)||0); }
...
async function gpuInfo(){
  ...
  if (await cmdExists('nvidia-smi')){
    const q = ['--query-gpu=name,memory.total,compute_cap','--format=csv,noheader'];
  ...
  if (await cmdExists('rocminfo')){
    ...
    const cards = (r.stdout.match(/Name:\s*(gfx[0-9a-z]+)/gi)||[]).map((x,i)=>({vendor:'amd', index:i, name:x.split(':')[1].trim()}));
    return { type:'amd', gpus: cards, raw: r.stdout.slice(0,5000) };
  }
  // Apple
  if (isMac) {
    return { type:'apple', gpus:[{vendor:'apple', index:0, name:'Apple GPU'}] };
  }
...
async function llamaListDevices(cli){
  ...
  const devs = [];
  const re = /Device\s+(\d+):\s+(.+?),\s+compute capability\s+([0-9.]+)/ig;
  for(;;){
...
async function tryNgl(cli, model, ngl){
  const args = ['-m', model, '--prompt', 'ping', '--n-predict', '8', '-ngl', String(ngl), '--no-perf'];
  const r = await run(cli, args, { timeoutMs: 60_000 });
  if (!r.ok) {
    const s = (r.stderr + r.stdout).toLowerCase();
    if (/out of memory|oom|cuda error|failed to malloc|cublas/i.test(s)) return { ok:false, reason:'oom' };
...
function sanitizeName(s){ return String(s).replace(/[/ :]+/g,'-').replace(/^-+|-+$/g,''); }

async function main(){
  const args = argParse();
  if (!args.model || !fs.existsSync(args.model)) {
    console.error('ERROR: --model /abs/path/model.gguf is required (or env LLAMACPP_MODEL_PATH).');
    process.exit(2);
  }

  const llamaCli = process.env.LLAMACPP_CLI || './llama.cpp/build/bin/llama-cli';
  ...
  if (args.quick) {
    ...
    if (memGiB>0){
      ...
      ngl = Math.max(0, Math.min(layersGuess, Math.floor((usable / fileMB) * layersGuess)));
    ...
    ngl = memGiB>=24 ? Math.min(layersGuess, 70) : (memGiB>=12 ? Math.min(layersGuess, 40) : 0);
  }
  ...
  const cli = {
    cmd: llamaCli,
    args: ['-m', args.model, '--n-predict','64','-ngl', String(ngl), '--threads', String(threads), '--flash-attn', flash],
    env
  };
  ...
  if (args.printCmd) {
    ...
    const sCmd = `${envStr} ${server.cmd} ${server.args.map(x=>(/[^A-Za-z0-9._:-]/.test(x)?JSON.stringify(x):x)).join(' ')}`.trim();
