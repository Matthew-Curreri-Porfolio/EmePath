...
export function makeTask({ id, userId, goal, inputs = {}, constraints = {}, priority = 0 }) {
  if (!id || !goal) throw new Error('task requires id and goal');
  return { id, userId: userId || 'anon', goal, inputs, constraints, priority };
}

export function makeMessage({ id, taskId, from, to, role = 'agent', content = '', data = {}, ts }) {
  return {
...
export function makeOutcome({ taskId, status = 'success', artifacts = {}, rationale = '' }) {
  return { taskId, status, artifacts, rationale };
}

export function makeConsensus({ roomId, members = [], method = 'majority', evidence = [], decision = {} }) {
