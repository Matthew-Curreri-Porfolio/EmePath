...
import {
  saveWhitePaper,
  getWhitePaperByPath,
  deleteWhitePaperByPath,
} from '../models/whitePaper.js';

const WHITE_PAPERS_DIR = path.join(
  process.cwd(),
  'gateway',
  'rooms',
  'white_papers'
...
async function processWhitePapers() {
  const files = fs
    .readdirSync(WHITE_PAPERS_DIR)
    .filter((file) => file.endsWith('.md'));

  // Track existing white papers to identify deletions
  const existingWhitePapers = new Set(
    (await getAllWhitePapers()).map((wp) => wp.filePath)
  );
