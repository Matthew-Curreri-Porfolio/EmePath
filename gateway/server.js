...
if (!process.env.GATEWAY_PORT)
  process.env.GATEWAY_PORT = String(CFG.ports.gateway);
...
if (String(process.env.FORECAST_AUTORESOLVE).toLowerCase() === 'true') {
  ...
  const tick = async () => {
    try {
      ...
      log({
        event: 'forecast_autoresolve',
        resolved: result?.resolved,
        sample: Array.isArray(result?.details)
          ? result.details.slice(0, 5)
          : [],
      });
    } catch (e) {
      log({
        event: 'forecast_autoresolve_error',
  ...
  if (
    String(process.env.FORECAST_AUTORESOLVE_ON_BOOT).toLowerCase() === 'true'
  ) {
    tick();
  }
}
