...
export default function registerRoutes(app, deps) {
  ...
  app.use((req, res, next) => {
    ...
    res.on('finish', () => {
      const routeLabel =
        req.route && req.route.path ? req.route.path : req.path;
      const dur = Date.now() - t0;
      httpMs
        .labels(routeLabel, req.method, String(res.statusCode))
        .observe(dur);
  ...
  try {
    ...
    const intervalMs = Number(
      process.env.CACHE_PURGE_INTERVAL_MS || 5 * 60_000
    );
    if (intervalMs > 0) {
      setInterval(() => {
        try {
  ...
  app.post(
    '/memory/compact/short-to-long',
    requireAuth,
    compressLimiter,
    validate(CompressionSchema),
    async (req, res) => {
      await (
        await import('../usecases/compress.js')
      ).compressShortToLongUseCase(req, res, deps);
    }
  );
  app.post(
    '/memory/compact/long',
    ...
    async (req, res) => {
      await (
        await import('../usecases/compress.js')
      ).compressLongGlobalUseCase(req, res, deps);
    }
  );
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      const safe =
        typeof req.query.safe !== 'undefined'
          ? String(req.query.safe).toLowerCase() === 'true' ||
            req.query.safe === true
          : undefined;
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      try {
        ...
        if (
          (process.env.CURATED_MODE || '').toLowerCase() === '1' ||
          (process.env.CURATED_MODE || '').toLowerCase() === 'true'
        ) {
          const cache = process.env.CURATED_CACHE;
          const cnum = num || 5;
          const c = await searchCuratedLocal(query, {
            num: cnum,
            site: req.query.site,
            lang: req.query.lang,
            cache,
          ...
          if (c && c.ok && Array.isArray(c.results) && c.results.length) {
            const mapped = c.results.map((r) => ({
              ...
              page: {
                ...
                description: '',
                headings: [],
                wordCount: (r.snippet || '').split(/\s+/).filter(Boolean)
                  .length,
                content: r.snippet || '',
              },
            }));
  ...
  app.get('/insights/graph', graphLimiter, async (req, res) => {
    try {
      ...
      if (!parsed.success)
        return res
          .status(400)
          .json({ ok: false, error: parsed.error.flatten() });

      const query = req.query.q || req.query.query;
      const mode =
        req.query.mode &&
        ['web', 'local', 'hybrid'].includes(String(req.query.mode))
      ...
      const localK = req.query.localK
        ? Math.min(Math.max(1, Number(req.query.localK)), 20)
        : undefined;
      const opts = {
        mode,
        base: process.env.SEARXNG_BASE,
        ...
        localK,
        maxContextChars: req.query.maxContextChars
          ? Number(req.query.maxContextChars)
          : undefined,
        maxAnswerTokens: req.query.maxAnswerTokens
          ? Number(req.query.maxAnswerTokens)
          : undefined,
      };
  ...
  app.get('/debate', debateLimiter, async (req, res) => {
    try {
      ...
      const mode =
        data.mode && ['web', 'local', 'hybrid'].includes(String(data.mode))
          ? String(data.mode)
          : 'hybrid';
      const num = (() => {
        const n = Number(data.n ?? data.num);
        return Number.isFinite(n) && n > 0 ? Math.min(n, 20) : undefined;
      })();
      const fetchNum = (() => {
        const n = Number(data.f ?? data.fetchNum);
        return Number.isFinite(n) && n > 0 ? Math.min(n, 10) : undefined;
      })();
      const concurrency = (() => {
        const n = Number(data.c ?? data.concurrency);
        return Number.isFinite(n) && n > 0 ? Math.min(n, 6) : undefined;
      })();
      const localK = data.localK
        ? Math.min(Math.max(1, Number(data.localK)), 20)
        : undefined;

      const opts = {
        mode,
        useInsights:
          typeof data.useInsights !== 'undefined'
            ? String(data.useInsights).toLowerCase() === 'true' ||
              data.useInsights === true
            : true,
        rounds: data.rounds ? Math.min(Math.max(1, Number(data.rounds)), 4) : 2,
        trace:
          typeof data.trace !== 'undefined'
            ? String(data.trace).toLowerCase() === 'true' || data.trace === true
            : false,
        base: process.env.SEARXNG_BASE,
        num,
        fetchNum,
        ...
        lang: data.lang,
        safe:
          typeof data.safe !== 'undefined'
            ? String(data.safe).toLowerCase() === 'true' || data.safe === true
            : undefined,
        fresh: data.fresh,
        localIndex: typeof getIndex === 'function' ? getIndex() : undefined,
        localK,
        maxContextChars: data.maxContextChars
          ? Number(data.maxContextChars)
          : undefined,
        maxAnswerTokens: data.maxAnswerTokens
          ? Number(data.maxAnswerTokens)
          : undefined,
      };
    ...
    try {
      ...
      const mode =
        data.mode && ['web', 'local', 'hybrid'].includes(String(data.mode))
          ...
          : 'hybrid';
      const num = data.num
        ? Math.min(Math.max(1, Number(data.num)), 20)
        : undefined;
      const fetchNum = data.fetchNum
        ? Math.min(Math.max(1, Number(data.fetchNum)), 10)
        : undefined;
      const concurrency = data.concurrency
        ? Math.min(Math.max(1, Number(data.concurrency)), 6)
        : undefined;
      const localK = data.localK
        ? Math.min(Math.max(1, Number(data.localK)), 20)
        : undefined;
    ...
    try {
      ...
      const opts = {
        ...
        iterations: data.iterations
          ? Math.min(Math.max(1, Number(data.iterations)), 10)
          : 2,
        perIter: data.perIter
          ? Math.min(Math.max(1, Number(data.perIter)), 10)
          : 2,
        ...
        maxAnswerTokens: data.maxAnswerTokens
          ...
          : undefined,
        persist:
          typeof data.persist !== 'undefined'
            ? String(data.persist).toLowerCase() === 'true' ||
              data.persist === true
            : false,
  ...
  app.post('/forecast/seed', forecastLimiter, async (req, res) => {
    try {
      ...
      const opts = {
        ...
        lang: body.lang,
        safe:
          typeof body.safe !== 'undefined'
            ? String(body.safe).toLowerCase() === 'true' || body.safe === true
            : undefined,
        fresh: body.fresh,
        localIndex: typeof getIndex === 'function' ? getIndex() : undefined,
        localK: body.localK,
  ...
  app.post('/forecast/resolve', forecastLimiter, async (req, res) => {
    try {
      const { ForecastResolveSchema } = await import(
  ...
  app.get('/forecast/metrics', forecastLimiter, async (req, res) => {
    try {
      const { ForecastMetricsSchema } = await import(
        '../validation/schemas.js'
      );
      ...
      if (!parsed.success)
        return res
          .status(400)
          .json({ ok: false, error: parsed.error.flatten() });
      const { forecastMetrics, metricsToCsv, metricsForUi } = await import(
        '../tools/forecast.js'
      );
  ...
  app.post('/forecast/backtest/seed', forecastLimiter, async (req, res) => {
    try {
      const { ForecastBacktestSeedSchema } = await import(
        '../validation/schemas.js'
      );
      const body = req.body || {};
  ...
  app.get('/prompts/preview', (req, res) => {
    try {
      ...
      if (!text)
        return res
          .status(404)
          .json({ ok: false, error: 'unknown key or empty prompt' });
