...
function resolveFromModelfile(mfPath, maxDepth = 4) {
  ...
  for (let i = 0; i < maxDepth; i++) {
    ...
    if (from.startsWith("/") && isGGUF(from)) {
      return { path: from, source: "modelfile", chain, adapters: directives.ADAPTER || [] };
    }
    // relative gguf
    const rel = path.join(dir, from);
    if (isGGUF(rel)) {
      return { path: rel, source: "modelfile", chain, adapters: directives.ADAPTER || [] };
    }
    // sha256
    if (/^(sha256-)?[0-9a-f]{64}$/i.test(from)) {
      const digest = from.replace(/^sha256-/, "");
      const p = tryBlobPaths(digest);
      if (p) return { path: p, source: "modelfile->blob", chain, adapters: directives.ADAPTER || [] };
    }
    // id:tag
    if (from.includes(":")) {
      const p = resolveFromManifests(from);
      if (p) return { path: p, source: "modelfile->manifest", chain, adapters: directives.ADAPTER || [] };
    }
    // follow nested Modelfile if 'FROM' points to another file
    const asPath = from.startsWith("/") ? from : path.join(dir, from);
    const looksLikeModelfile = /modelfile$/i.test(asPath) || path.basename(asPath).toLowerCase() === "modelfile";
    ...
    if (guess) return { path: guess, source: "modelfile->dir", chain, adapters: directives.ADAPTER || [] };
    break;
  }
  return null;
}
...
export function resolveModelPath(arg) {
  ...
  // 1) absolute GGUF
  if (arg.startsWith("/") && isGGUF(arg)) return { path: arg, source: "direct" };
