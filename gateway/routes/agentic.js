...
export function registerAgentic(app, deps, limiters = {}) {
    ...
    async (req, res) => {
      try {
        ...
        const num = req.query.num
          ? Math.min(Math.max(1, Number(req.query.num)), 20)
          : 5;
        const cache = process.env.CURATED_CACHE;
        const out = await searchCuratedLocal(q, {
          num,
          site: req.query.site,
          lang: req.query.lang,
          cache,
  ...
  app.get(
    ...
    async (req, res) => {
      ...
      if (!query)
        return res.status(400).json({ ok: false, error: 'missing_query' });
      const num = req.query.num
        ? Math.min(Math.max(1, Number(req.query.num)), 10)
        : 5;
      const fetchNum = req.query.fetchNum
        ? Math.min(Math.max(1, Number(req.query.fetchNum)), 6)
        : 3;
      const concurrency = req.query.concurrency
        ? Math.min(Math.max(1, Number(req.query.concurrency)), 6)
        : 3;
  ...
  app.post('/rooms/dispatch', async (req, res) => {
    try {
      const body = req.body || {};
      const { defaultDeps, runTask, Protocols } = await import(
        '../rooms/index.js'
      );
      const goal = String(body.goal || '').trim();
      if (!goal)
        return res.status(400).json({ ok: false, error: 'goal is required' });
      const task = Protocols.makeTask({
  ...
  app.post('/db/maintain', async (req, res) => {
    try {
      ...
      const reencode =
        String((req.body && req.body.reencode) || '').toLowerCase() ===
          'true' ||
        (req.body && req.body.reencode === true);
  ...
  app.post('/db/backup', async (_req, res) => {
    try {
      ...
      await new Promise((resolve, reject) => {
        const child = spawn('bash', ['-lc', 'scripts/backup_db.sh'], {
          cwd: process.cwd(),
        ...
        child.on('close', (code) =>
          code === 0 ? resolve(last) : reject(new Error(last || 'exit ' + code))
        );
      });
      res.json({ ok: true });
    } catch (e) {
